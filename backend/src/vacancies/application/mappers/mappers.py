...
from src.vacancies.domain.schemas import VacancySource, VacancyCreate


class OriginalVacancyMapper:
...
class DomainVacancyMapper:
    ...
    def _map_vacancy_to_domain(self, vacancy: TVacancy | dict) -> Vacancy:
        ...
        if isinstance(vacancy, dict):
            vacancy = OriginalVacancyMapper(self.source).map_one(vacancy)
...
class DomainToDBMapper:
    """Преобразует доменную модель к модели базы данных"""
    def map(self, vacancies: list[Vacancy]) -> list[VacancyCreate]:
        """Маппинг списка TVacancy к списку VacancyCreate"""
        return [self._map_domain_vacancy_to_db_schema(vacancy) for vacancy in vacancies]

    def map_one(self, vacancy: Vacancy) -> VacancyCreate:
        return self._map_domain_vacancy_to_db_schema(vacancy)

    def _map_domain_vacancy_to_db_schema(self, vacancy: Vacancy) -> VacancyCreate:
