...
from src.integrations.headhunter.schemas.response import HHVacancyResponse, HHVacancy
from src.integrations.interfaces import IVacancySourceClient


class HeadHunterClient(APIClient, IVacancySourceClient[HHVacancySearchParams, HHVacancy, HHVacancyResponse]):
    ...
    def __init__(
        ...
        super(HeadHunterClient, self).__init__(source_url, auth_type=auth_type, token=token)
    ...
    async def get_all_vacancies(
        ...
        for i in range(max_pages):
            ...
            _vacancies = await self.get_vacancies(search_params)
            if _vacancies:
    ...
    async def _get_vacancy_response(self, search_params: HHVacancySearchParams) -> HHVacancyResponse:
        """
        Получаем сначала только Response, который содержит более подробную информацию,
        включая вакансии, их количество, текущую страницу и прочее.
        """
        headers = {"HH-User-Agent": f"JobScope/1.0 {settings.EMAIL_FROM}"}
        response = await self.request(
